    ...
    pub fn most_visited_pattern(
        ...
        for i in 0..n {
            timestamp_username_website.push((
                timestamp[i],
                String::from(&username[i]),
                String::from(&website[i]),
            ));
        }
        ...
        for i in 0..n {
            match username_vs_websites.get_mut(&timestamp_username_website[i].1) {
                Some(w) => w.push(String::from(&timestamp_username_website[i].2)),
                None => {
        ...
        for (_, v) in username_vs_websites.into_iter() {
            ...
            for path in paths.iter() {
                pattern_counter.insert(
                    String::from(path),
                    pattern_counter.get(path).unwrap_or(&0) + 1,
                );
                ...
                } else if *(pattern_counter.get(path).unwrap()) == most_pattern_max_score
                    && most_pattern.ge(path)
                {
                    most_pattern = String::from(path);
                }
            }
...
mod test {
    ...
    #[test]
    fn case02() {
        let username = to_vec_string(["zkiikgv", "zkiikgv", "zkiikgv", "zkiikgv"].iter());
        let timestamp = vec![436363475, 710406388, 386655081, 797150921];
        let website = to_vec_string(["wnaaxbfhxp", "mryxsjc", "oz", "wlarkzzqht"].iter());
